...
watch(selectedStore, (newState, prevState) => {
  if (prevState && !newState) refresh();
});
</script>

<template>
  <main
    :class="[useRouter().currentRoute.value.meta.classNames]"
    class="m-0 h-100vh grid grid-cols-5 p-0 merchant"
  >
    <div class="block relative col-span-3 h-100vh overflow-y-scroll">
      <header
        class="p-8 flex items-center justify-between text-gray-800 mb-2 sticky top-0 z-10 bg-gray-50"
      >
        <RouterLink v-slot="{ navigate }" to="/tickets" custom>
          <button
            ...
            :class="{
              invisible:
                useRouter().currentRoute.value.fullPath === '/merchant',
            }"
            @click="navigate"
          >
        ...
        <h2>Your Stores</h2>
        <RouterLink
          to="/merchant/stores/create"
          class="text-white bg-emerald-500 rounded-lg p-2"
        >
      ...
      <div v-else class="p-8">
        <section class="grid grid-cols-2 gap-12">
          <div
            ...
            :key="store.id"
            class="relative block border border-gray-200 rounded-lg overflow-hidden"
          >
            <img
              :alt="store.name"
            ...
            <div class="p-6">
              <div class="h-20">
                <h5 class="text-xl font-bold">
                  ...
                  <span
                    :class="{
                      'bg-red-500': store.status === 'stopService',
                      'bg-emerald-500': store.status === 'onService',
                    }"
                    class="inline-block text-white rounded-md px-2 py-1 text-xs font-normal mb-1 uppercase opacity-90 mx-2"
                    v-text="
                      store.status === 'onService'
                        ? 'In Service'
                        : 'Out of Service'
                    "
                  />
              ...
              <div class="flex items-center space-x-6 mt-8">
                ...
                <a
                  ...
                  @click="
                    updateStoreDetailStore.setSelectedStore({
                      selectedStore: {
                        ...store,
                        phoneNumbersText: store.phoneNumbers.join(`\n`),
                      },
                    })
                  "
                >
