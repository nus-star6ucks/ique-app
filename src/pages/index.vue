...
const { data: stores, loading: isLoading } = useRequest(() =>
  storeApi.storesListGet().then((d) => d.data)
);
...
<template>
  <div class="space-y-4 mb-20">
    <header class="flex items-center justify-between mb-12">
      <h1 class="text-xl font-medium">iQue</h1>
    ...
    <div v-if="!userStore.user" class="p-6 rounded-lg bg-white relative">
      <div>
        <h2 class="text-2xl uppercase mb-4 font-light">Welcome to iQue</h2>
        <RouterLink v-slot="{ navigate }" to="/me" custom>
          <button
            class="border rounded-lg p-2 border-gray-800 flex items-center text-sm"
            @click="navigate"
          >
            <span>Sign up or login to start</span>
