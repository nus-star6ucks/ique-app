...
function confirmLogout() {
  ...
  if (window.confirm("Are you sure?")) userStore.logout();
}
...
function confirmDelete() {
  if (!userStore.user || deleteLoading.value) return;
  // eslint-disable-next-line no-alert
  const userInput = window.prompt(
    "This action cannot be undone, please type your username to continue: "
  );
  if (userInput === userStore.user.username) deleteMySelf(+userStore.user.id);
}
</script>
