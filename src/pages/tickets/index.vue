...
const { data: tickets, loading: isLoading } = useRequest(async () => {
  ...
  return Promise.all(
    tickets.map(async (ticket) => {
      const { data: storeData } = await storeApi.storesStoreIdGet(
        ticket.storeId
      );
      const { data: queueData } = await queueApi.queuesTicketsTicketIdGet(
        ticket.ticketId
      );
...
<template>
  <WithAuth>
    <header class="flex items-center justify-between mb-8">
      <h1 class="text-xl font-medium">Tickets</h1>
        ...
        >
          ...
          <div class="col-span-3">
            <div class="my-4 p-6 bg-white rounded-xl">
              <h3
                class="mt-2 text-lg text-gray-800 font-bold"
                v-text="`${ticket.queueNumber} - ${ticket.seatType.name}`"
              />
              <p
                class="mt-2 text-sm text-gray-500"
                v-text="ticket.store.name"
              />
            </div>
          </div>
        </RouterLink>
